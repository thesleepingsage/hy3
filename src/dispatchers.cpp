...
SDispatchResult dispatch_swapwindow(std::string value) {
	...
	if (args.size() < 2) {
		hy3_log(Log::ERR, "hy3:swapwindow requires two window addresses");
	...
	if (!windowA || !windowB) {
		hy3_log(Log::ERR, "hy3:swapwindow could not find one or both windows");
		return { .success = false, .error = "could not find one or both windows" };
	}

	if (windowA == windowB) {
		hy3_log(Log::DEBUG, "hy3:swapwindow same window, ignoring");
		return { .success = true };
	}

	hy3_log(Log::DEBUG, "hy3:swapwindow swapping {} and {}", (void*)windowA.get(), (void*)windowB.get());
